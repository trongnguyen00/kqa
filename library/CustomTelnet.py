...
class CustomTelnet():
    ...
    ``CustomTelnet`` is a customized version of Robot Framework's standard Telnet library
    that makes it possible to connect to Telnet servers and execute commands on the opened connections.
    This version allows for inheritance and customization while maintaining all original functionality.
    ...
    The first step of using ``CustomTelnet`` is opening a connection with `Open
    ...
    def __init__(
        ...
        """CustomTelnet library can be imported with optional configuration parameters.
        ...
        | Library     | CustomTelnet    |                          |                      |                     | # default values |
        | Library     | CustomTelnet    | 5 seconds                |                      |                     | # set only timeout |
        | Library     | CustomTelnet    | newline=LF               | encoding=ISO-8859-1  |                     | # set newline and encoding using named arguments |
        | Library     | CustomTelnet    | prompt=$                 |                      |                     | # set prompt |
        | Library     | CustomTelnet    | prompt=(> |# )           | prompt_is_regexp=yes |                     | # set prompt as a regular expression |
        | Library     | CustomTelnet    | terminal_emulation=True  | terminal_type=vt100  | window_size=400x100 | # use terminal emulation with defined window size and terminal type |
        | Library     | CustomTelnet    | telnetlib_log_level=NONE |                      |                     | # disable logging messages from the underlying telnetlib |
        """
    ...
    def _get_connection(self, *args):
        ...
        return CustomTelnetConnection(*args)
    ...
    def send_commands_from_group(self, file_path, command_group, **variables):
        """
        Send Commands From Group    resource.yaml    setup-olt    mapper=1    vlan=202
        or
        Send Commands From Group    resource.yaml    setup-olt    @{dict}
        """
        handler = TelnetCommandGroupHandler(self._conn)
        handler.send_commands_from_group(file_path, command_group, variables)

    @keyword
    def get_current_prompt(self):
        """Return current prompt"""
        prompt, is_regexp = self._conn._prompt
...
class CustomTelnetConnection(telnetlib.Telnet):
...
class TelnetCommandGroupHandler:
    """Handler for executing groups of telnet commands from YAML files."""
    ...
    def _adjust_prompt_level(self, target):
        ...
        while self.current_prompt_level < target:
            # Giả định prompt sẽ tăng khi gửi lệnh (đã nằm trong logic send)
            self.telnet.read_until_regexp(r"(#|>)")
            self.current_prompt_level += 1
